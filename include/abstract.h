...
public:
    S_symbol sym;
    A_SimpleVar(A_pos p, S_symbol s) : A_var(p), sym(s) {};
};
...
public:
    ...
    S_symbol sym;
    A_FieldVar(A_pos p, std::unique_ptr<A_var> var_, S_symbol s) : A_var(p), var(std::move(var_)), sym(s) {};
};
...
public:
    ...
    A_SubscriptVar(A_pos p, std::unique_ptr<A_var> var_, std::unique_ptr<A_exp> exp_) : 
                                    A_var(p), var(std::move(var_)), exp(std::move(exp_)) {};
};
...
public:
    A_NilExp(A_pos p) : A_exp(p) {};
};

struct A_IntExp : public A_exp {
public:
    int i;
    A_IntExp(A_pos p, int i_) : A_exp(p), i(i_) {};
};

struct A_StringExp : public A_exp {
public:
    std::string s;
    A_StringExp(A_pos p, std::string s_) : A_exp(p), s(s_) {};
};
...
public:
    ...
    A_CallExp(A_pos p, S_symbol func_, std::unique_ptr<A_expList> args_) :
                A_exp(p), func(func_), args(std::move(args)) {};
};
...
public:
    ...
    A_OpExp(A_pos p, A_oper oper_, std::unique_ptr<A_exp> left_, std::unique_ptr<A_exp> right_) :
        A_exp(p), oper(oper_), left(std::move(left_)), right(std::move(right_)) {};
};
...
public:
    ...
    A_RecordExp(A_pos p, S_symbol type_, std::unique_ptr<A_efieldList> fields_) :
        A_exp(p), type(type_), fields(std::move(fields_)) {};
};
...
public:
    ...
    A_AssignExp(A_pos p, std::unique_ptr<A_var> var_, std::unique_ptr<A_exp> exp_) :
        A_exp(p), var(std::move(var_)), exp(std::move(exp_)) {};
};
...
public:
    ...
    A_IfExp(A_pos p, std::unique_ptr<A_exp> test_, std::unique_ptr<A_exp> then_, std::unique_ptr<A_exp> elsee_) :
        A_exp(p), test(std::move(test_)), then(std::move(then_)), elsee(std::move(elsee_)) {};
};
...
public:
    ...
    A_WhileExp(A_pos p, std::unique_ptr<A_exp> test_, std::unique_ptr<A_exp> body_) :
        A_exp(p), test(std::move(test_)), body(std::move(body_)) {};
};

struct A_BreakExp : public A_exp {
public:
    A_BreakExp(A_pos p) : A_exp(p) {};
};
...
public:
    ...
    A_ForExp(A_pos p, S_symbol var_, std::unique_ptr<A_exp> lo_, std::unique_ptr<A_exp> hi_, std::unique_ptr<A_exp> body_) :
        A_exp(p), var(var), lo(std::move(lo_)), hi(std::move(hi_)), body(std::move(body_)) {};
};
...
public:
    ...
    A_LetExp(A_pos p, std::unique_ptr<A_decList> decs_, std::unique_ptr<A_exp> body_) :
        A_exp(p), decs(std::move(decs_)), body(std::move(body_)) {};
};
...
public:
    ...
    A_ArrayExp(A_pos p, std::unique_ptr<A_exp> size_, std::unique_ptr<A_exp> init_) :
        A_exp(p), size(std::move(size_)), init(std::move(init_)) {};
};
...
public:
    ...
    A_FunctionDec(A_pos p, std::unique_ptr<A_funcdecList> function_) :
        A_dec(p), function(std::move(function_)) {}; 
};
...
public:
    ...
    A_VarDec(A_pos p, S_symbol var_, S_symbol type_, std::unique_ptr<A_exp> init_) :
        A_dec(p), var(var_), type(type_), init(std::move(init_)) {};
};
...
public:
    S_symbol type;
    A_NameTy(A_pos p, S_symbol type_) : A_ty(p), type(type_) {};
};
...
public:
    ...
    A_RecordTy(A_pos p, std::unique_ptr<A_fieldList> record_) :
        A_ty(p), record(std::move(record_)) {};
};

struct A_ArrayTy : public A_ty {
public:
    S_symbol array;
    A_ArrayTy(A_pos p, S_symbol array_) : A_ty(p), array(array_) {};
};
